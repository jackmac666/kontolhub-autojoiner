...
-- Discord button click handler - Enhanced copy with proper status management
discordButton.MouseButton1Click:Connect(function()
    print("💬 KONTOL HUB: Discord button clicked!")
    
    -- KONTOL HUB Discord link for copying
    local discordText = "discord.gg/kontolhub"
    local discordInvite = "https://discord.gg/kontolhub"
    
    -- Use statusLabel directly since updateStatus is defined later
    statusLabel.Text = "💬 Copying Discord link..."
    print("💬 KONTOL HUB: Attempting to copy Discord link to clipboard...")
    ...
    pcall(function()
        ...
        if setclipboard then
            ...
            copySuccess = true
            print("✅ KONTOL HUB: Discord link copied using setclipboard")
        elseif toclipboard then
            toclipboard(discordInvite)
            copySuccess = true
            print("✅ KONTOL HUB: Discord link copied using toclipboard")
        elseif Clipboard and Clipboard.set then
            Clipboard.set(discordInvite)
            copySuccess = true
            print("✅ KONTOL HUB: Discord link copied using Clipboard.set")
        elseif writeclipboard then
            writeclipboard(discordInvite)
            copySuccess = true
            print("✅ KONTOL HUB: Discord link copied using writeclipboard")
        end
    end)
    ...
    if copySuccess then
        ...
        print("✅ KONTOL HUB: Discord link successfully copied to clipboard!")
    else
        statusLabel.Text = "💬 Manual: " .. discordText
        jobBox.Text = "Manual copy: " .. discordInvite
        print("⚠️ KONTOL HUB: Failed to copy Discord link, showing manual copy")
    end
    
    -- Clear message after 4 seconds and restore status
    spawn(function()
        wait(4)
        statusLabel.Text = "🚀 KONTOL HUB - Ready for ZERO DELAY joining!"
        jobBox.Text = ""
        print("🔄 KONTOL HUB: Discord copy message cleared, status restored")
    end)
end)
...
local function startHttpPolling()
    httpPolling = true
    print("⚡ KONTOL HUB: Starting HTTP clipboard polling mode...")
    spawn(function()
                    ...
                    if isAutojoinerEnabled then
                        ...
                        spawn(function()
                            ...
                            if not teleportSuccess then
                                print("❌ KONTOL HUB: Teleport failed - " .. tostring(teleportErr))
                                print("🔄 KONTOL HUB: Server likely full, staying connected for next Job ID...")
                                updateStatus("❌ Server full! Waiting for next Job ID...")
                                
                                -- Clear the failed job ID and stay connected
                                spawn(function()
                                    wait(3)
                                    if isConnected then
                                ...
                                print("✅ KONTOL HUB: Successfully teleporting to server!")
                            end
                        end)
                    end
                end
        ...
        print("⚪ KONTOL HUB: HTTP polling stopped")
    end)
end
    ...
    spawn(function()
        local success, err = pcall(function()
                ...
                if jobId and jobId ~= "" then
                    ...
                    if isAutojoinerEnabled then
                        ...
                        spawn(function()
                            ...
                            if not teleportSuccess then
                                print("❌ KONTOL HUB: WebSocket teleport failed - " .. tostring(teleportErr))
                                print("🔄 KONTOL HUB: Server likely full, staying connected for next Job ID...")
                                updateStatus("❌ Server full! Waiting for next Job ID...")
                                ...
                                spawn(function()
                                    wait(3)
                                    if isConnected then
                                        updateStatus("✅ WebSocket Connected! Waiting for Copy Job ID...")
                                        jobBox.Text = ""
                                    end
                                end)
                            else
                                updateStatus("✅ REAL-TIME joining server...")
                                print("✅ KONTOL HUB: Successfully teleporting to server via WebSocket!")
                            end
                        end)
                    end
                end
            ...
            socket.OnClose:Connect(function()
                print("⚠️ KONTOL HUB: WebSocket connection lost!")
                isConnected = false
                isAutojoinerEnabled = false
                httpPolling = false
                updateButtonState()
                updateStatus("❌ Connection lost! Click CONNECT to retry")
            end)
            
        end)
        
        if not success and not isConnected then
            print("❌ KONTOL HUB: Connection failed - " .. tostring(err))
            print("💡 KONTOL HUB: Make sure Python server is running!")
            isConnected = false
            httpPolling = false
            toggleIcon.Text = "⚡"
            toggleText.Text = "CONNECT"
            ...
            updateStatus("❌ Connection failed - Check Python server!")
        end
    end)
end
...
local function disconnectFromServer()
    print("🔌 KONTOL HUB: Disconnecting from server...")
    if socket then
        pcall(function() socket:close() end)
        socket = nil
    end
    ...
    print("⚪ KONTOL HUB: Successfully disconnected!")
end
...
toggleButton.MouseButton1Click:Connect(function()
    if not isConnected then
        print("🔄 KONTOL HUB: Attempting to connect...")
        ...
        print("🔌 KONTOL HUB: User requested disconnect...")
...
manualHopBtn.MouseButton1Click:Connect(function()
    ...
    print("🌐 KONTOL HUB: Manual server hop initiated...")
    ...
    if not success then
        print("❌ KONTOL HUB: Manual hop failed - " .. tostring(err))
        updateStatus("❌ Manual hop failed: " .. tostring(err))
        
        -- Reset status after failed manual hop
        spawn(function()
            wait(3)
            if isConnected then
                updateStatus("⚡ Ready for next Job ID!")
        ...
        print("✅ KONTOL HUB: Manual hop successful!")
    end
end)
